...
public class FCS16 {
    ...
    static short FCS16_INIT = (short) (0xffff);
    static short FCS16_GOOD = (short) (0xf0b8);
    ...
    static short HIGHBYTE(short val) {
        return (short) ((val >> 8) & 0x00ff);
    }

    static short LOWBYTE(short val) {
        return (short) (val & 0x00ff);
    }

    private static short fcs16_Generate(short fcs, ByteBuffer cp) {
        short ret = fcs;
        for (int i = 0; i < cp.position(); i++) {
            ret = fcs16_AddByte(ret, cp.get(i));
        }
        return (ret);
    }

    static short fcs16_AddByte(short fcs, byte c)
    {
        short high = HIGHBYTE(fcs);
        short low = LOWBYTE(fcs);
        short s = (short) c;
        s &= 0x00ff;
        return (short) (high ^ fcstab[low ^ s]);
    }

    static short fcs16_Add(ByteBuffer cp) {
        short trialfcs = fcs16_Generate(FCS16_INIT, cp);
        trialfcs ^= 0xffff;
        return trialfcs;
    }

    static boolean fcs16_Verify(ByteBuffer cp) {
        short trialfcs;

        trialfcs = fcs16_Generate(FCS16_INIT, cp);
        return (trialfcs == FCS16_GOOD);
    }
}
